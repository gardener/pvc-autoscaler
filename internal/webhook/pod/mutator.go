...
// autoscalerName of this pvc-autoscaler.
...
func (m *Mutator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	if m.shouldGate(ctx, logger, pod) {
		utils.AddSchedulingGate(pod, common.SchedulingGateOfflineResize)
		logger.Info("adding offline-resize scheduling gate to pod", "pod", client.ObjectKeyFromObject(pod))
	}
...
// scheduling gate.
func (m *Mutator) shouldGate(ctx context.Context, logger logr.Logger, pod *corev1.Pod) bool {
	for _, volume := range pod.Spec.Volumes {
		...
		pvcKey := client.ObjectKey{Namespace: pod.Namespace, Name: volume.PersistentVolumeClaim.ClaimName}
		if err := m.client.Get(ctx, pvcKey, pvc); err != nil {
			if apierrors.IsNotFound(err) {
				continue
			}

			logger.Error(err, "failed to get PersistentVolumeClaim referenced by pod, not adding scheduling gate", "pvc", pvcKey)
